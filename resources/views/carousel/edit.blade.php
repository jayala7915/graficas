...
@section('title', 'Editar Item de Carousel')
            ...
            <form method="POST" action="{{ route('carousel.update', $carousel->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="titulo_id" name="titulo_id" required>
                            @foreach($titulos as $id => $descripcion_titulo)
                                <option value="{{ $id }}" {{ $carousel->titulo_id == $id ? 'selected' : '' }}>{{ $descripcion_titulo }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                        <label for="texto" class="form-label">Texto breve</label>
                        <input type="text" class="form-control" id="texto" name="texto" value="{{ old('texto', $carousel->texto) }}" required>
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <div class="mt-2">
                            <img src="{{ asset('storage/'.$carousel->imagen) }}" width="100" class="img-thumbnail">
                            <small class="text-muted">Imagen actual</small>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <label for="ruta" class="form-label">Ruta/Identificador</label>
                        <input type="text" class="form-control" id="ruta" name="ruta" value="{{ old('ruta', $carousel->ruta) }}" required>
                    </div>
                </div>
                
                <div class="mb-3">
                    <label for="texto_amplio_html" class="form-label">Contenido enriquecido</label>
                    <textarea id="jodit-editor" name="texto_amplio_html">{{ old('texto_amplio_html', $carousel->texto_amplio_html) }}</textarea>
                </div>
                
                <div class="mb-3 form-check">
                    <input type="checkbox" class="form-check-input" id="activo" name="activo" {{ $carousel->activo ? 'checked' : '' }}>
